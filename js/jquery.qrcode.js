...
for (function(a) {
    a.fn.qrcode = function(b) {
        ...
        d = function() {
            ...
            //     }
            // }
            // 修改后=>解决生成table二维码时，宽高被取整后导致二维码变形的问题。
            for (c.addData(utf16to8(b.text)), c.make(), d = a("<table></table>").css("width", b.width + "px").css("height", b.height + "px").css("border", "0px").css("border-collapse", "collapse").css("background-color", b.background), e = b.width / c.getModuleCount(), f = b.height / c.getModuleCount(), g = 0; g < c.getModuleCount(); g++) {
                for (h = a("<tr></tr>").css("height", Math.floor(f) + "px").appendTo(d), i = 0; i < c.getModuleCount(); i++) {
                    a("<td></td>").css("width", Math.floor(e) + "px").css("background-color", c.isDark(g, i) ? b.foreground: b.background).appendTo(h)
                }
            }
            // 通过计算tr标签的数量和宽高，修改table标签的宽高，确保二维码不会变形。
			var len = d.find('tr')[0].childElementCount
			d.css('width',Math.floor(e)*len).css('height',Math.floor(f)*len)
            return d
        },
        this.each(function() {
